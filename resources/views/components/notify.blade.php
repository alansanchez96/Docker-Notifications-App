@auth
    ...
    <div id="dropdownNotification" class="z-20 hidden w-full max-w-sm bg-white divide-y divide-gray-100 rounded-b-lg shadow"
        ...
        <div class="block px-4 py-2 text-xl text-center font-bold text-rose-500 bg-gray-900">
            Notificaciones
        </div>
            ...
            @if (count(auth()->user()->notifications) > 0)
                @if (count(auth()->user()->unreadNotifications) > 0)
                    ...
                    @if (count(auth()->user()->readNotifications) > 0)
                        <div class="block px-4 py-2 font-bold text-xl text-center text-rose-500 bg-gray-900">
        ...
        <a href="{{ route('markAsRead') }}"
            class="block py-2 text-sm font-bold text-center text-gray-100 rounded-b-lg bg-gray-900 hover:text-rose-500">
            <div class="inline-flex items-center ">
                <svg class="w-4 h-4 mr-2 mt-1 text-rose-500" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20"
